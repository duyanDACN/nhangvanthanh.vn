    ...
    <div class="item__tel">
        ...
        <div class="content__tel">
            ...
            <div class="tel"><a href="tel:<?php echo $tel_tel; ?>"><?php echo $tel_tel; ?></a> |
                <a href="tel:<?php echo $tel_tel2; ?>"><?php echo $tel_tel2; ?></a>
            </div>
        </div>
    </div>
    <?php
...
add_action('wp_ajax_update_checkout_address', 'ajax_update_checkout_address');
add_action('wp_ajax_nopriv_update_checkout_address', 'ajax_update_checkout_address');

function ajax_update_checkout_address()
{
    if (!is_user_logged_in()) {
        ...
        if (email_exists($email)) {
            ...
            wc()->mailer()->customer_new_account($user_id, $password);
        }
    }
    ...
    foreach ($fields as $field) {
        if (isset($_POST[$field])) {
            ...
            $post_value = sanitize_text_field($_POST[$field]);

            if (($field === 'billing_country' || $field === 'shipping_country') && empty($post_value)) {
                $post_value = 'VN';
            }

            update_user_meta($user_id, $field, $post_value);

            if (str_starts_with($field, 'billing_')) {
                $shipping_key = str_replace('billing_', 'shipping_', $field);
                update_user_meta($user_id, $shipping_key, $post_value);
            }

            // SET VÀO CHECKOUT (CÓ CHECK METHOD)
            if( $checkout && method_exists($checkout, 'set_value') ){
                $checkout->set_value($field, $post_value);
            }
        }
    }
...
add_action('woocommerce_checkout_update_order_meta', function( $order_id, $data ) {
    ...
    foreach($fields as $f){
        $v = get_user_meta($user_id, $f, true);
        if($v){
            update_post_meta($order_id, '_' . $f, $v);
        }
    }

}, 10, 2);
//Tắt giao hàng địa chỉ khác
